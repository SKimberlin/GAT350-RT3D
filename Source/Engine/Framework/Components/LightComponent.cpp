...
namespace nc
{
	...
	void LightComponent::Read(const nc::json_t& value)
	{
		// read json file
		std::string typeName;
		if (READ_NAME_DATA(value, "lightType", typeName))
		{
			if (IsEqualIgnoreCase(typeName, "point")) type = Point;
			if (IsEqualIgnoreCase(typeName, "directional")) type = Directional;
			if (IsEqualIgnoreCase(typeName, "spot")) type = Spot;
		}
